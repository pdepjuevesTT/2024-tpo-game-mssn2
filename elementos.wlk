...
const offset = 13
object izquierda{ 
	method siguiente(position) = position.left(grilla.movimientoEnX()) 
	method opuesto() = derecha
}

object derecha{ 
	method siguiente(position) = position.right(grilla.movimientoEnX()) 
	method opuesto() = izquierda
}

object abajo{ 
	method siguiente(position) = position.down(grilla.movimientoEnY()) 
	method opuesto() = arriba
}

object arriba{ 
	method siguiente(position) = position.up(grilla.movimientoEnY()) 
...
object estadoCero inherits EstadoBarra(rango=0,numero=1){
    override method estaEnRango(_unValor) = true
}


class BarraGeneral inherits Visual(image = null){
    ...
    method inicializarEstadosBarra(){
        const a = (15..1).map({ i => self.estado(i) }) 
        estados.addAll(a)
        estados.add(estadoCero)
    }

    method estado(i) = new EstadoBarra(rango = i * 6.25, numero = i + 1 )

    method actualizarBarra(parametro) {
        porcentajeActual = (parametro / parametroTotal) * 100
        estNuevo = "barra" + nombre + self.numeroDeEstado(porcentajeActual) + ".png"
